...
\column{0.45\textwidth}
  ...
  \begin{itemize}
    ...
    \medskip
    \item Two competing forces:
    \begin{itemize}
      \item[\textcolor{mygreen}{$\uparrow$}] Economy grows ($\bar Y > Y_0$) --- helps everyone
      \item[\textcolor{myred}{$\downarrow$}] Extra supply ($f{+}s > f$) --- hurts exposed workers
    \end{itemize}

    \medskip
    \item \textbf{Deep inside $[0,I^*)$:} extra supply dominates
          $\rightarrow$ \textcolor{myred}{wages fall}

    \medskip
    \item \textbf{Near or above $I^*$:} output growth dominates
          $\rightarrow$ \textcolor{mygreen}{wages rise}
...
\textbf{Workers \emph{heavily} exposed to AI ($s/f$ large) face two pressures:}

\bigskip
\begin{columns}[t]
\column{0.48\textwidth}
  \textbf{\textcolor{myred}{Wage compression}}\\[6pt]
  AI floods their task market with supply --- scarcity falls faster than the economy grows $\rightarrow$ lower pay
...
{\small
\textit{Workers near $I^*$ (mild exposure) can still \textcolor{mygreen}{gain} --- the output
expansion outpaces the small supply addition there. The split is \textbf{not}
at $I^*$ but somewhere below it.}\\[4pt]
\color{mygray} Augmentation channel (AI raising worker productivity) is set aside.}
...
\medskip
\begin{itemize}
  \item \textbf{Numerator} $\bar Y > Y_0$: economy grew $\rightarrow$ pushes wages \textcolor{mygreen}{up} for all
  ...
  \item For $i$ near $I^*$: $s(i)\approx 0$, output growth still dominates $\rightarrow$ wages \textcolor{mygreen}{rise}
  \item For $i$ deep inside $[0,I^*)$: $s/f$ large, denominator outpaces numerator $\rightarrow$ wages \textcolor{myred}{fall}
\end{itemize}
...
\smallskip
{\small The crossover between gain and compression depends on $s(i)/f(i)$ vs.\ $\bar Y/Y_0 - 1$.\\
Workers near $I^*$ can gain even below the frontier; deep exposure is needed for compression.}
