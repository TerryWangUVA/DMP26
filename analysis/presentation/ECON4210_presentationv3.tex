...
% --- Top/Bottom exposure ---
\begin{frame}{Who's Most / Least Exposed?}
...
\column{0.37\textwidth}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item 2025: $+0.018^{***}$
    \end{itemize}
    \item $\approx$ 6pp increase relative to sample mean of 3.4\% 
...
\column{0.37\textwidth}
  ...
  \begin{itemize}
    ...
    \medskip
    \item Perhaps wage adjustment lags behind employment adjustment
  \end{itemize}
\end{columns}
...
% LIMITATIONS
% =============================================================================
\begin{frame}{Before You Panic\ldots}

\textbf{Why the observed unemployment is probably an \textcolor{myred}{underestimate} for the actual friction:}
\begin{itemize}
  \item CPS-ASEC captures unemployment - but misses workers who got
        laid off and found another job quickly
  \item No data on job openings or new grads struggling to enter the market (\textit{Some more seasoned researchers did try to analyze these}) 
\end{itemize}

\bigskip
\textbf{Why you should \textcolor{mygreen}{not} panic:}
\begin{itemize}
  \item A lot has been going on since 2020 --- hard to isolate AI from
        everything else
  \item Pre-trends aren't perfectly clean (thanks, COVID) --- though the
        2020--21 pattern actually \emph{confirms} the ``cognitive tasks''
        story in a weird way
  \item Only 2--3 post-treatment years so far
  \item Even if it's an underestimate, the effect is still quite mild
\end{itemize}
\end{frame}
...
\centering
\vspace{2cm}
{\large\color{mygray}\itshape Backup: Derivation of Wage Schedulecc }\\[0.5cm]
\end{frame}
