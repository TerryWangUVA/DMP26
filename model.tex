...
low end are more easily performed by AI, while tasks at the high end require human efforts that AI currently struggles
to replicate.  This ordering maps naturally to the \citet{eloundou2024gpts} exposure scores: a high exposure score indicates that a large share of an occupation's
tasks could be susceptible to large language models, placing that occupation's task bundle near
...
\begin{equation}
  ...
  \label{eq:ai_cost}
\end{equation}
where $\tau > 0$ is the ``AI capability'' parameter: a lower $\tau$
...
\begin{assumption}[Exogenous Wage]
  The wage $w$ is determined outside the model.  The economy is small and
  faces an infinitely elastic labor supply at wage $w$
\end{assumption}
