...
**Within-occupation vs. between-occupation.** Before offering explanations, I examine whether the college/non-college wage asymmetry arises *within* the same high-exposure occupations --- that is, whether college and non-college workers employed in the same occupation have different wage responses to AI --- or whether it arises *between* occupations, because the two groups sort into systematically different high-exposure occupations in the first place. I pool the two education groups and estimate

$$Y_{it} = \alpha + \sum_{t \neq 2022} \delta_t \left( \mathbf{1}[t] \times \text{AIExposure}_i \times \text{College}_i \right) + \text{lower-order terms} + X_{it}' \Gamma + \mu_{o,t} + \varepsilon_{it},$$

where $\mu_{o,t}$ is an occupation-by-year fixed effect that absorbs the entire between-occupation component of the exposure effect (including the year $\times$ exposure main effect, since $\beta_o$ varies only at the occupation-year level). The triple-interaction coefficients $\{\hat\delta_t\}$ are identified by the within-occupation-year gap between college and non-college workers at the same level of AI exposure. If $\{\hat\delta_t\}$ is statistically indistinguishable from zero post-2022, the wage asymmetry cannot be explained by differential responses of college and non-college workers to AI exposure *within the same occupation*; it must instead reflect the fact that the two groups sort into different high-exposure occupations.

Figure [FIG:LW_WITHIN_PLOT] plots the $\{\hat\delta_t\}$ sequence for log wages, and Table [TAB:HET_WITHIN] reports the regression. [RESULT PLACEHOLDER: if $\{\hat\delta_t\}$ is null post-2022, write: "The triple-interaction coefficients are small and statistically indistinguishable from zero across all post-2022 years, indicating that the college/non-college wage gap documented above does not arise within the same occupations. It is driven by between-occupation differences --- college-heavy and non-college-heavy high-exposure occupations responding differently to AI." If $\{\hat\delta_t\}$ is significant, write: "The triple-interaction coefficients are significant in 2024 and 2025, indicating that the college/non-college gap arises within the same occupations: college workers in a given high-exposure occupation fare systematically better than non-college workers in that same occupation."]

The test result narrows the space of plausible explanations. [WRITE conditional on above: if between-occupation dominates, drop Explanation 1 or demote it heavily; if within-occupation dominates, foreground Explanation 1.] With this ruling-out step in place, I offer two candidate explanations for the asymmetric wage response.

*Explanation 1: within-bundle time reallocation.* Remark 1 in Section 2 held the bundle weights $\theta^x$ fixed by assumption. If workers can instead adjust $\theta^x$ in response to the shock — reallocating time away from AI-substitutable tasks and toward tasks where AI does not substitute — the wage effect on the re-weighted bundle is attenuated. College-educated workers in high-exposure occupations may be better positioned on this margin, either because their bundles contain a larger number of tasks with heterogeneous exposure or because they have more flexibility in how time is divided across the bundle. Neither claim is tested in this paper; both are offered as conjecture. \textcite{hampole2025ai} develop a formal framework in which workers reallocate effort from AI-exposed to unexposed tasks within their occupations, with the capacity to offset task displacement determined by how concentrated exposure is across tasks. \textcite{autor2025expertise} offer a related angle: when automation removes the easier tasks from an occupation's bundle, what remains demands more of the judgment and domain knowledge that workers acquire through schooling and experience. Additional years of education plausibly confer some of that advantage, partly shielding college-educated workers in exposed occupations.

*Explanation 2: demand-side shifts not captured in the model.* A second and more conservative interpretation attributes the wage asymmetry to output-market effects the toy model does not capture. Generative AI may have raised aggregate demand for the output produced by college-heavy high-exposure occupations: rising productivity in software, legal services, and financial analysis can translate into larger demand for these services, or into new product categories that employ workers in these occupations. Non-college-heavy high-exposure occupations — customer service, data entry, routine office work — do not obviously benefit from a comparable demand-side boost; if anything, their output is itself what firms increasingly automate away. Under this reading, the wage asymmetry reflects a rightward shift in labor demand for college-heavy exposed occupations that partly or fully offsets the supply-dilution pressure identified in Section 2, while non-college-heavy exposed occupations absorb the supply shock without a corresponding demand offset. \textcite{chandar2025tracking}'s finding that employment has grown in software development and contracted in customer service within the most exposed quartile is especially consistent with this demand-side channel, which predicts employment-level shifts of the kind a pure within-bundle reassignment story does not.

Both explanations are coherent with the symmetry of the unemployment response alongside the asymmetry in wages. Extensive-margin adjustments — slower hiring into exposed roles and layoffs of marginal workers — operate at the firm-and-occupation level, where the pressure on the firm's wage bill is determined by occupation-level exposure and need not differ sharply by the education composition of the workforce. Intensive-margin wage compression, in contrast, requires the compression pressure to pass through to individual pay; whether it does depends on whether the worker can preserve productivity through within-bundle reallocation (Explanation 1) or on whether output-market demand shores up labor demand for the occupation (Explanation 2). The two explanations are not mutually exclusive; distinguishing them and quantifying their relative contributions is left to future work.
